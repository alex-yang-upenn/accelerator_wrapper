...
# 30 "/home/ayvol/accelerator_wrapper/src/defines.h"
...
# 91 "/home/ayvol/accelerator_wrapper/src/alveo_hls4ml.h"
...
void myproject(
    input_t input_5[4*4*3],
    result_t layer11_out[5]
);
# 5 "/home/ayvol/accelerator_wrapper/src/alveo_hls4ml.cpp" 2


static void read_input(const input_data_t *in, input_data_t (&in_buf)[8192][(4 * 4 * 3)]) {
  VITIS_LOOP_8_1: for (int i = 0; i < 8192; i++) {
#pragma HLS PIPELINE
 VITIS_LOOP_10_2: for(int j = 0; j < (4 * 4 * 3); j++) {
#pragma HLS UNROLL
 in_buf[i][j] = in[i * (4 * 4 * 3) + j];
      }
    }
}
static void run_inference(input_data_t (&in_buf)[8192][(4 * 4 * 3)], output_data_t (&out_buf)[8192][5]) {
...
# 102 "/home/ayvol/accelerator_wrapper/src/alveo_hls4ml.cpp"
      ...
      input_data_t in_buf[8192][(4 * 4 * 3)];
