...
typedef struct {
    ...
    XAlveo_hls4ml_uio_map maps[ MAX_UIO_MAPS ];
} XAlveo_hls4ml_uio_info;

/***************** Variable Definitions **************************************/
static XAlveo_hls4ml_uio_info uio_info;
...
static int uio_info_read_name(XAlveo_hls4ml_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAlveo_hls4ml_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAlveo_hls4ml_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAlveo_hls4ml_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAlveo_hls4ml_Initialize(XAlveo_hls4ml *InstancePtr, const char* InstanceName) {
	XAlveo_hls4ml_uio_info *InfoPtr = &uio_info;
